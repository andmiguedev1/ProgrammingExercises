...
int iterativeBinarySearch(int givenArray[], int searchValue) 
{
    int lowPointer = givenArray[0];
    int lengthArray = sizeof(givenArray) / sizeof(lowPointer);
    int highPointer = lengthArray - 1;
    
    // Repeat search operation until a condition is met
    while (lowPointer <= highPointer) { 
        // Formula to calculate halfPointer of any array size
        int halfPointer = lowPointer + (highPointer - lowPointer) / 2; 
...
int main(void) 
{ 
    int exampleArray[] = { 3, 7, 12, 21, 28, 34 }; 
    int exampleValue = 28; 

    int searchResult = iterativeBinarySearch(exampleArray, exampleValue); 
